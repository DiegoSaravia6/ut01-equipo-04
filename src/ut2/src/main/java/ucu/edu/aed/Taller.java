...
public class Taller {
    ...
    public Taller() {
        ...
        Comparator<Vehiculo> criterioAtencion = (a, b) -> {
            ...
            // Los vehículos con compromiso siempre preceden
            // a los que no tienen fecha.
            if (fa != null && fb == null) {
                return -1;
            }
            ...
            // Entre comprometidos, la fecha más cercana
        ...
        vehiculosEnEspera =
                new MonticuloBinario<>(criterioAtencion);
        ...
        indiceVehiculosPorPatente =
                new ArbolAVL<>();

        indiceOrdenesPorFecha =
                new ArbolAVL<>();
    }
    ...
    private OrdenTrabajo crearOrden(
            Vehiculo vehiculo,
            LocalDate fechaIngreso) {
    ...
    private void completarIngreso(
            Vehiculo vehiculo,
            LocalDate fechaEntregaComprometida) {

        vehiculo.configurarPrioridad(
                fechaEntregaComprometida,
                siguienteOrdenLlegada++
        );

        indiceVehiculosPorPatente.insertar(
                vehiculo
        );

        vehiculosEnEspera.insertar(
                vehiculo
        );
    }

    private void validarVehiculoParaIngreso(
            Vehiculo vehiculo) {
    ...
    private void validarDetalleIngreso(
            String detalle) {
    ...
    private void validarFechas(
        ...
        if (fechaEntrega != null
                && fechaEntrega.isBefore(fechaIngreso)) {

            throw new IllegalArgumentException(
                    "La fecha comprometida no puede ser anterior al ingreso"
            );
        }
    }

    public Vehiculo proximoVehiculo() {
        return vehiculosEnEspera.primero();
    }
    ...
    public void reprogramarVehiculo(
            String patente,
            LocalDate nuevaFecha) {

        Vehiculo vehiculo =
                buscarVehiculoPorPatente(patente);
        ...
        if (!vehiculosEnEspera.contiene(vehiculo)) {
            throw new IllegalStateException(
                    "El vehículo ya no está en espera de atención"
            );
        }

        vehiculo.reprogramarEntrega(
                nuevaFecha
        );

        vehiculosEnEspera.reordenar(
                vehiculo
        );
    }
    ...
    public Vehiculo buscarVehiculoPorPatente(
        ...
        return indiceVehiculosPorPatente.buscar(
                v -> clave.compareToIgnoreCase(
                        v.getPatente()
                )
        );
    }

    public ListaArreglo<OrdenTrabajo> buscarOrdenesEntre(
            LocalDate desde,
            LocalDate hasta) {
        ...
        ListaArreglo<RegistroOrdenFecha> registros =
                indiceOrdenesPorFecha.listarRango(
                        RegistroOrdenFecha.limiteInferior(desde),
                        RegistroOrdenFecha.limiteSuperior(hasta)
                );

        ListaArreglo<OrdenTrabajo> resultado =
                new ListaArreglo<>();
    ...
    public void registrarTallerista(
    ...
    public Vehiculo atenderSiguiente() {
        ...
        Vehiculo vehiculo =
                vehiculosEnEspera.extraerPrimero();

        tallerista.asignarVehiculo(
                vehiculo
        );

        vehiculosEnTrabajo.agregar(
                vehiculo
        );
    ...
    public Reparacion registrarTrabajoPrincipal(
        ...
        return orden.agregarTrabajoPrincipal(
                descripcion,
                parte,
                costo,
                aprobado
    ...
    public Reparacion registrarTrabajoDerivado(
        ...
        return orden.agregarTrabajoDerivado(
                idTrabajoOrigen,
                descripcion,
                parte,
                costo
    ...
    public void aprobarTrabajo(
            Vehiculo vehiculo,
            int idTrabajo) {

        ordenDe(vehiculo)
                .aprobarTrabajo(idTrabajo);
    }

    public void rechazarTrabajo(
            Vehiculo vehiculo,
            int idTrabajo) {

        ordenDe(vehiculo)
                .rechazarTrabajo(idTrabajo);
    }
    ...
    public void suspenderTrabajoPorRepuesto(
        ...
        orden.suspenderTrabajoPorRepuesto(
                idTrabajo
        );

        /*
         * IMPORTANTE:
         * Si todavía existe algún trabajo ejecutable
         * en otra rama, el vehículo permanece en trabajo.
         */
        if (!orden.tieneTrabajoEjecutable()) {

            Tallerista tallerista =
                    buscarTalleristaAsignado(
                            vehiculo
                    );

            vehiculosEnTrabajo.remover(
                    vehiculo
            );

            esperandoRepuestos.poneEnCola(
                    vehiculo
            );
    ...
    public void reanudarTrabajoPorRepuesto(
        ...
        ordenDe(vehiculo)
                .reanudarTrabajoPorRepuesto(
                        idTrabajo
                );
    }

    public ListaArreglo<Reparacion> consultarOrdenPendiente(
            Vehiculo vehiculo) {

        return ordenDe(vehiculo)
                .ordenPendiente();
    }

    public ListaArreglo<Reparacion> consultarTrabajosBloqueados(
            Vehiculo vehiculo,
            int idTrabajo) {

        return ordenDe(vehiculo)
                .trabajosBloqueadosPor(
                        idTrabajo
                );
    }
    ...
    public double costoAutorizadoOrden(
            Vehiculo vehiculo) {

        return ordenDe(vehiculo)
                .costoAutorizadoTotal();
    }

    public double costoRama(
            Vehiculo vehiculo,
            int idTrabajo) {

        return ordenDe(vehiculo)
                .costoPropuestoSubarbol(
                        idTrabajo
                );
    }

    private OrdenTrabajo ordenDe(
            Vehiculo vehiculo) {

        if (vehiculo == null
                || vehiculo.getOrdenActual() == null) {

            throw new IllegalArgumentException(
                    "El vehículo no tiene una orden activa"
            );
        }
    ...
    private ParteVehiculo parteDe(
        ...
        ParteVehiculo parte =
                vehiculo.buscarParte(
                        codigoParte
                );

        if (parte == null) {
            throw new IllegalArgumentException(
                    "No existe la parte " + codigoParte
            );
        }
    ...
    public void finalizarVehiculo(
        ...
        if (orden != null
            ...
            throw new IllegalStateException(
                    "La orden de trabajo tiene ramas pendientes"
            );
        }
        ...
        vehiculosProntos.agregar(
                vehiculo
        );
    }

    private Tallerista buscarTalleristaAsignado(
            Vehiculo vehiculo) {

        Tallerista tallerista =
                talleristas.buscar(
                        t -> t.getVehiculoActual()
                                == vehiculo
                );
    ...
    public boolean estaProntoParaRetirar(
            Vehiculo vehiculo) {

        return vehiculosProntos.contiene(
                vehiculo
        );
    }
}